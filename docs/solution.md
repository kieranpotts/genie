...
| Command execution control           | `bash` replacement (via `audited-tools`) never invokes a shell, rejects control operators, runs only allowlisted programs, and refuses any argument resolving into the mediated project path. Self-enforced and lexical — see the extension README for its stated limits. |
